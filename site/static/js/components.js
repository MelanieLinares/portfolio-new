...
(function() {
	...
	Modal.prototype.showModal = function() {
        ...
        // Ensure modal container is focusable to act as a fallback trap boundary
	...
	Modal.prototype.trapFocus = function(event) {
		// If focus is at the last element (close button) and user hits Tab (forward), wrap to first element.
        ...
        // If Shift is pressed (backward trap)
        if(event.shiftKey) {
            
            // Condition A: If focus is on the first focusable element (expected starting point for backward escape)
            // Condition B: OR if focus has already escaped the entire modal wrapper (iframe failure mode)
            var shouldWrapBackward = (
  ...
  // this is new:
//   Modal.prototype.checkFocus = function() {
//     var self = this;
//     setTimeout(function() {
//         // If the modal is visible AND the active element is outside the modal, 
//         // pull focus back to the last element (the Close button).
//         if (Util.hasClass(self.element, self.showClass) && !self.element.contains(document.activeElement)) {
//             self.lastFocusable.focus();
//         }
//     }, 1); // A minimal timeout is all that's usually needed
// };
// end new
...
(function() {
  ...
  function initAdvSelect(select) {
    ...
    Util.removeClass(select.trigger, 'hidden');
    Util.removeClass(select.dropdown, 'hidden');
  };
  ...
  function initAdvSelectEvents(select) {
    ...
    select.dropdown.addEventListener('keydown', function(event){
      ...
      if(event.keyCode && (event.keyCode == 38 || event.keyCode == 40 || event.keyCode == 37 || event.keyCode == 39) || event.key && (event.key.toLowerCase() == 'arrowup' || event.key.toLowerCase() == 'arrowdown' || event.key.toLowerCase() == 'arrowleft' || event.key.toLowerCase() == 'arrowright')) {
        
        // Prevent the default browser action (like scrolling/closing)
        event.preventDefault(); 
        
        var direction;
        // Map Up (38) / Left (37) to 'prev'
        if(event.keyCode == 38 || event.key.toLowerCase() == 'arrowup' || event.keyCode == 37 || event.key.toLowerCase() == 'arrowleft') {
            direction = 'prev';
        } 
        // Map Down (40) / Right (39) to 'next'
        else {
            direction = 'next';
        }
        
        keyboardCustomSelect(select, direction);
      ...
      } else if(event.key && event.key.toLowerCase() == ' ' || event.keyCode && event.keyCode == 32) {
        // Space key to select
        event.preventDefault();
        triggerSelection(select, document.activeElement);
      }
  ...
  function getOptionCode(select, option, optionClone) {
    ...
    // FIX: Always set tabindex="0" so all options are keyboard focusable
  ...
  function selectOption(select, option) {
    if(option.hasAttribute('aria-selected') && option.getAttribute('aria-selected') == 'true') {
      ...
      if(selectedOption) {
        ...
        // selectedOption.removeAttribute('tabindex'); // Not needed now that we always set it in getOptionCode
      }
      option.setAttribute('aria-selected', 'true');
      // option.setAttribute('tabindex', '0'); // Not needed now that we always set it in getOptionCode
  ...
  var flexHeader = document.getElementsByClassName('js-f-header');
	if(flexHeader.length > 0) {
		var menuTrigger = flexHeader[0].getElementsByClassName('js-anim-menu-btn')[0],
			...
			// RENAMED: This is the target for *initial focus*
			firstLinkInMenu = getFirstLinkInMenu(); 

		// NEW: These will store the boundaries for the focus trap
		var trapFirstFocusable = null,
			trapLastFocusable = null;

		// we'll use these to store the node that needs to receive focus when the mobile menu is closed 
		var focusMenu = false;

		updateFocusTrapElements(); // NEW: Call on init
		...
		// Listen for tab key (focus trap) on keydown
		window.addEventListener('keydown', function(event){
			if( (event.keyCode && event.keyCode == 9) || (event.key && event.key.toLowerCase() == 'tab' )) {
				// trap focus inside navigation
				if(menuTrigger.getAttribute('aria-expanded') == 'true' && isVisible(menuTrigger)) {
					if ( event.shiftKey ) { // Shift + Tab
						// CHANGED: Use trap variables
						if ( document.activeElement === trapFirstFocusable ) { 
							event.preventDefault();
							if(trapLastFocusable) trapLastFocusable.focus();
						}
					} else { // Tab
						// CHANGED: Use trap variables
						if ( document.activeElement === trapLastFocusable ) { 
							event.preventDefault();
							if(trapFirstFocusable) trapFirstFocusable.focus();
						}
					}
				}
			}
		...
		// RENAMED: This function finds the first link *in the nav*
		function getFirstLinkInMenu() {
		...
		// NEW: This function finds the first/last elements *in the whole header* for the trap
		function updateFocusTrapElements() {
			// CHANGED: Query is now scoped to the entire header (flexHeader[0])
			var focusableEle = flexHeader[0].querySelectorAll('[href], input:not([disabled]), select:not([disabled]), textarea:not([disabled]), button:not([disabled]), iframe, object, embed, [tabindex]:not([tabindex="-1"]), [contenteditable], audio[controls], video[controls], summary');
			
			trapFirstFocusable = false;
			trapLastFocusable = false;
			var visibleFocusable = [];

			for(var i = 0; i < focusableEle.length; i++) {
				if( isVisible(focusableEle[i]) ) {
					visibleFocusable.push(focusableEle[i]);
				}
			}

			if(visibleFocusable.length > 0) {
				trapFirstFocusable = visibleFocusable[0]; // This will be the logo
				trapLastFocusable = visibleFocusable[visibleFocusable.length - 1]; // This will be the last link
			}
    };
    ...
    function isVisible(element) {
      return (element.offsetWidth || element.offsetHeight || element.getClientRects().length);
		...
		function doneResizing() {
			...
			resetFlexHeaderOffset();
			updateFocusTrapElements(); // CHANGED: Call trap function
		};
		
		function toggleMenuNavigation(bool) { // toggle menu visibility on small devices
			// 1. Toggle classes first
			Util.toggleClass(document.getElementsByClassName('f-header__nav')[0], 'f-header__nav--is-visible', bool);
			Util.toggleClass(flexHeader[0], 'f-header--expanded', bool);
			menuTrigger.setAttribute('aria-expanded', bool);
			
			// 2. Update the trap boundaries *after* elements are visible/hidden
			updateFocusTrapElements(); 

			if(bool) {
				// 3. Move focus to the first *link*, not the logo
				if(firstLinkInMenu) firstLinkInMenu.focus(); 
			} else if(focusMenu) {
				focusMenu.focus();
				focusMenu = false;
			}
		...
		if(slideshow.options.navigation) {
			...
			if(slideshow.element.getElementsByClassName('js-slideshow__navigation').length == 0) {
				...
				for(var i = 0; i < slideshow.items.length; i++) {
					...
					navChildren = navChildren + '<li '+className+'><button class="appearance-none bg-transparent"><span class="sr-only">'+ (i+1) + '</span>'+navCurrentLabel+'</button></li>';
				}
